...
def CopyTable(sheet,maxrow=1000,maxcol=1000):
    ...
    Returns table from the worksheet as a list of lists. The first sublist are headers. The rest of sublists is data.
    """
    hedaers=GetHeaders(sheet,maxrow,maxcol)
    data_length=len(headers)
    table_copy=[]
    flag_found_stm=False
    it = -1
    for row in sheet.iter_rows(min_row=1, min_col=1, max_row=maxrow+1, max_col=maxcol+1):
        ...
        it+=1
        for cell in row:
            #wsadzam tylko te pola ktore nie sa puste
            if cell.value is not None:
                flag_in_row = True
                data_row_pos+=1
                flag_found_stm = True     #juz wiem, ze trafilem na tabele z danymi
                table_copy[it].append(cell.value)
            elif data_row_pos<data_length and flag_in_row == True:  #jesli trafimy na puste pole w miejscu danych
                data_row_pos +=1
                table_copy[it].append(cell.value)
            if data_row_pos >= data_length: #skonczylismy iterowac po danych, dalej sa puste miejsca
                break
        if table_copy[it] == []:
            del table_copy[-1]
            it-=1
            if flag_found_stm == True:  #trafilem na tabele z danymi, wszedlem tu poniewaz ostatni rzad byl pusty
                return table_copy       #tzn. koniec tabeli, nie musze dalej iterowac
    return table_copy
	'''